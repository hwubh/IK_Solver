...
namespace UnityEngine.PathTracing.Core
{
    ...
    internal class MaterialPool : IDisposable
    {
        ...
        private readonly Dictionary<int, MaterialEntry> _materials = new();
        private readonly Dictionary<int, BlockAllocator.Allocation> _lightCookies = new();  // this dictionary is used to keep track of which textures are inserted in the atlas and avoid duplication (important for LiveGI)
        private readonly Dictionary<int, int> _cookieHandleToID = new();                    // this dictionary is used when deleting lights with cookies attached, so we can free-up the atlas slots
        ...
        public void AddMaterial(int materialHandle, in World.MaterialDescriptor material, UVChannel albedoAndEmissionUVChannel)
        {
        ...
        public void UpdateMaterial(int materialHandle, in World.MaterialDescriptor material, UVChannel albedoAndEmissionUVChannel)
        {
            MaterialEntry materialEntry = _materials[materialHandle];
            UpdateMaterial(in material, albedoAndEmissionUVChannel, materialEntry);
        }

        public void RemoveMaterial(int materialInstanceID)
        {
        ...
        public uint GetMaterialIndex(int materialHandle)
        {
            MaterialEntry entry;
            if (_materials.TryGetValue(materialHandle, out entry))
                return (uint)entry.IndexInBuffer.block.offset;
            else
                throw new InvalidOperationException("Material not in pool");
        }

        public bool IsEmissive(int materialInstanceID, out float3 emissionColor)
        {
            float3 zero = new(0, 0, 0);
            emissionColor = zero;

            if (_materials.TryGetValue(materialInstanceID, out var entry))
            {
        ...
        public int AddCookieTexture(Texture cookie)
        {
            if (cookie != null)
            {
                var cookieID = cookie.GetInstanceID();
