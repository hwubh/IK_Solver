...
namespace UnityEngine.PathTracing.Integration
{
    internal class UnityComputeProbeIntegrator : IProbeIntegrator
    {
        ...
        public UnityComputeProbeIntegrator(bool excludeMeshAndEnvironmentFromDirect, bool countNEERayAsPathSegment)
        {
            _probeIntegrator = new ProbeIntegrator(excludeMeshAndEnvironmentFromDirect, countNEERayAsPathSegment);
        }
        ...
        public void Prepare(IDeviceContext context, IWorld world, BufferSlice<Vector3> positions, float pushoff, int bounceCount)
        {
            ...
            RayTracingContext rayTracingContext = _world.RayTracingContext;

            ProbeIntegrator.ResourceLibrary resources = new();
#if UNITY_EDITOR
            resources.Load(rayTracingContext);
            _probeOcclusionLightIndexMappingShader = AssetDatabase.LoadAssetAtPath<ComputeShader>("Packages/com.unity.path-tracing/Runtime/Shaders/ProbeOcclusionLightIndexMapping.compute");
            _probeOcclusionLightIndexMappingKernel = _probeOcclusionLightIndexMappingShader.FindKernel("MapIndices");
#endif

            _probeIntegrator.Prepare(unifiedContext.GetComputeBuffer(positions.Id), resources);
        }
