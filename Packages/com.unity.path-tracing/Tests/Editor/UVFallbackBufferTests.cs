...
namespace UnityEngine.PathTracing.Tests
{
            ...
            for (int y = 0; y < resolution.height; ++y)
            {
                for (int x = 0; x < resolution.width; ++x)
                {
                    ...
                    if (x < uvBoundsResolution && y < uvBoundsResolution)
                        Assert.Greater(color.r, 0.0f, $"Texel [{x}, {y}] should get as the location falls within the UV bounds.");
                    else
    ...
    internal class UVSamplingTests
    {
        ...
        public void SetUp()
        {
            ...
            _resources = new UVFallbackBufferResources(_backend);
            _gBufferShader = _resources._context.LoadRayTracingShader("Packages/com.unity.path-tracing/Runtime/Shaders/LightmapGBufferIntegration.urtshader");
            _samplingResources = new SamplingResources();
            _samplingResources.Load();
        }
        ...
        static private HitEntry[] GetSampleHitEntries(CommandBuffer cmd, IRayTracingShader gbufferShader, UVMesh uvMesh, UVAccelerationStructure uvAS, UVFallbackBuffer uvFB, SamplingResources samplingResources, Vector2Int instanceTexelOffset, int sampleCount, bool stochasticAntialiasing = true, uint superSamplingWidth = 1)
        {
            ...
            using GraphicsBuffer gBuffer = new GraphicsBuffer(GraphicsBuffer.Target.Structured | GraphicsBuffer.Target.CopySource, uvFB.Width * uvFB.Height, UnsafeUtility.SizeOf<HitEntry>());
            gbufferShader.SetBufferParam(cmd, LightmapIntegratorShaderIDs.GBuffer, gBuffer);
            uint expandedSize = (uint)(uvFB.Width * uvFB.Height);
            uvFB.BindChunked(cmd, gbufferShader, instanceTexelOffset, new uint2(0,0), expandedSize);
            ...
            float2 ssParams = new float2(0.5f, 0.5f);
            List<HitEntry> hitEntries = new();
            List<float3> float3Entries = new();
            for (int i = 0; i < sampleCount; ++i)
            {
                uint currentAAIndex = (uint)i % (superSamplingWidth * superSamplingWidth);
                gbufferShader.SetIntParam(cmd, LightmapIntegratorShaderIDs.AASampleOffset, i);
                if (!stochasticAntialiasing)
                    ssParams = SuperSamplingHelpers.GetSuperSamplingOffset(currentAAIndex, superSamplingWidth);
                ...
                gbufferShader.Dispatch(cmd, traceScratchBuffer, expandedSize, 1, 1);
                HitEntry[] hits = ReadBackHitEntries(cmd, gBuffer);
                hitEntries.AddRange(hits);
            }
        ...
        private static readonly (int width, int height, int sampleCount, float expectedDiscrepancy)[] SamplingVariations =
{
        ...
        public void Sample_SingleQuadIn_SamplesAreDistributed([ValueSource(nameof(SamplingVariations))] (int width, int height, int uvSampleCount, float expectedDiscrepancy) testData, [ValueSource(nameof(_bvhBuildFlags))] BuildFlags buildFlags)
        {
            ...
            // for this configuration of points we get size of 0.992402, 0.9979166
            float minSize = 0.99f;
        ...
        public void Sample_SingleQuad_SuperSamplesAreCorrect([ValueSource(nameof(SuperSamplingVariations))] (int width, int height, int uvSampleCount, uint superSamplingWidth, Vector3 expectedBoundsSize, Vector3 expectedBoundsOrigin, float expectedDiscrepancy) testData, [ValueSource(nameof(_bvhBuildFlags))] BuildFlags buildFlags)
        {
            ...
            Assert.AreEqual(expectedBoundsSize.x, pointBounds.size.x, 0.0001f, "Expected bounds size x is incorrect.");
            Assert.AreEqual(expectedBoundsSize.y, pointBounds.size.y, 0.0001f, "Expected bounds size y is incorrect.");
            Assert.AreEqual(expectedBoundsOrigin.x, pointBounds.min.x, 0.0001f, "Expected bounds origin x is incorrect.");
            Assert.AreEqual(expectedBoundsOrigin.y, pointBounds.min.y, 0.0001f, "Expected bounds origin y is incorrect.");
