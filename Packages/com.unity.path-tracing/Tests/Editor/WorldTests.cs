...
namespace UnityEngine.PathTracing.Tests
{
    ...
    internal class WorldTests
    {
        ...
        public void Setup()
        {
            ...
            _world = new World();
            World.ResourceLibrary worldResources = new World.ResourceLibrary();
            worldResources.Load();
        ...
        public void World_AddLight_IncreaseLightCount(int lightCount)
        {
            ...
            _world.cdfLightPicking = true;
            _world.Build(new Bounds(), _cmd, ref _buildScratchBuffer);
            Graphics.ExecuteCommandBuffer(_cmd);
            Assert.AreEqual(lightCount, _world.LightCount);
        }

        [Test]
        public void World_AddThreeLights_BuildsCorrectCDF()
        {
            int lightCount = 3;
            var lights = CreateLights(lightCount);
            _world.AddLights(lights, _respectLightLayers, _autoEstimateLUTRange, MixedLightingMode.IndirectOnly);
            ...
            _world.Build(new Bounds(), _cmd, ref _buildScratchBuffer);
            Graphics.ExecuteCommandBuffer(_cmd);
            float[] cdfReference = { 0.33333334f, 0.66666667f, 1.0f };
            var cdfValues = new float[lightCount];
            _world.LightCdfBuffer.GetData(cdfValues);
            for (int lightIndex = 0; lightIndex < lightCount; ++lightIndex)
            {
                Assert.AreEqual(cdfReference[lightIndex], cdfValues[lightIndex]);
            }
            Assert.AreEqual(lightCount, _world.LightCount);
        }

        [Test]
        public void World_AddNoLightsButEnableEmissiveSampling_EnvironmentIsAddedToCDF()
        {
            int lightCount = 0;
            var lights = CreateLights(lightCount);
            Assert.AreEqual(0, lights.Length);
            _world.AddLights(lights, _respectLightLayers, _autoEstimateLUTRange, MixedLightingMode.IndirectOnly);
            _world.cdfLightPicking = true;
            _world.EnableEmissiveSampling = true;
            _world.Build(new Bounds(), _cmd, ref _buildScratchBuffer);
            Graphics.ExecuteCommandBuffer(_cmd);
            float[] cdfReference = { 1.0f };
            var cdfValues = new float[1];
            _world.LightCdfBuffer.GetData(cdfValues);
            Assert.AreEqual(cdfReference[0], cdfValues[0]);
            int numEnvLights = 1;   // emissive sampling implicitly adds an env light for the skybox
            Assert.AreEqual(lightCount + numEnvLights, _world.LightCount);
        }
